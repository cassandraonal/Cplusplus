...
int main() {
    ...
    bool isLessOrEqual = number <= charValue;

    // Check if number <= -1 OR character value != 5 using logical operators
    bool conditionCheck = (number <= -1) || (charValue != 5);

    // Check if character value is between -2 and 2, inclusive
    bool isBetween = (charValue >= -2) && (charValue <= 2);

    // Print the results using \t to align answers
    cout<<"\tTriple the number:\t\t\t\t"<<number<<endl;
    cout<<"\tRemainder of the character:\t\t\t"<<charValue<<endl;
    cout<<"\tIs the number <= character value?\t\t"<<isLessOrEqual<<endl;
    cout<<"\tIs the number <= -1 or character != 5?\t\t"<<conditionCheck<<endl;
    cout<<"\tIs the character value between -2 and 2?\t"<<isBetween<<endl;
